...
<form class="login-form" action="/admin/login" method="post">
    ...
    <div class="alert alert-success">
        <button class="close" data-close="alert"></button>
        <span>{{ trans('login.sent_message') }}</span>
    </div>
    @endif
    @if (isset($message) && $message == "reset")
    <div class="alert alert-success">
        <button class="close" data-close="alert"></button>
        <span>{{ trans('login.reset_message') }}</span>
    </div>
    @endif
    @if (isset($message) && $message == "activated")
    <div class="alert alert-success">
        <button class="close" data-close="alert"></button>
        <span>{{ trans('login.activate_message') }}</span>
    </div>
    @endif
    <h3 class="form-title" style="color: #C00">{{ trans('login.title') }}</h3>
    <div class="form-group">
        <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
        <label class="control-label visible-ie8 visible-ie9">{{ trans('login.username') }}</label>
        <div class="input-icon">
            <i class="fa fa-user"></i>
            <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="{{ trans('login.username') }}" name="username"/>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="input-icon">
            ...
            <input class="form-control placeholder-no-fix" type="password" autocomplete="off" placeholder="{{ trans('login.password') }}" name="password"/>
        </div>
    </div>
    <div class="form-actions">
        <label class="checkbox">
            <input type="checkbox" name="remember" value="1" {{isset($remember) && $remember ? 'checked="checked"' : '' }} /> {{ trans('login.remember') }} </label>
        <button type="submit" class="btn blue pull-right">
            {{ trans('login.button') }} <i class="m-icon-swapright m-icon-white"></i>
        </button>
    </div>
    <!--
    ...
    <div class="forget-password">
        ...
        <p>
            {{ trans('login.no_worries') }} <a href="javascript:;" id="forget-password">{{ trans('login.here') }}</a> {{ trans('login.to_reset') }}.
        </p>
    </div>
    <!--
    <div class="create-account">
        <p>
            ¿ No tienes una cuenta de usuario ?&nbsp; <a href="javascript:;" id="register-btn"> Registrate </a>
        </p>
    </div>
    -->
</form>
...
<form class="forget-form" action="/admin/forgot" method="post">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <h3>{{ trans('login.forgot') }}</h3>
    <p>
        {{ trans('login.enter_email') }}
    </p>
    <div class="form-group">
        <div class="input-icon">
            <i class="fa fa-envelope"></i>
            <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="{{ trans('login.username') }}" name="email"/>
        </div>
    </div>
    <div class="form-actions">
        <button type="button" id="back-btn" class="btn">
            <i class="m-icon-swapleft"></i> {{ trans('login.back') }} </button>
        <button type="submit" class="btn blue pull-right">
        ...
        var Login = function () {
            ...
            var handleLogin = function() {
                $('.login-form').validate({
                    ...
                    messages: {
                        username: {
                            required: '<?php echo trans('login.validations.email_required') ?>',
                            email: '<?php echo trans('login.validations.invalid_email') ?>'
                        },
                        password: {
