...
function bs_citem( $params, $content=null ){
    ...
    $result .= '<a class="accordion-toggle '.($open!='true'?'collapsed':'').'" data-toggle="collapse" aria-controls="heading_' . $id . '" data-parent="#' . $parent . '" data-target="#' . $id . '" '.($open=='true'? 'aria-expanded="true"' : '').'>';
    $result .= $title;
    $result .= '</a>';
    $result .= '        </h4>';
    $result .= '    </div>';
